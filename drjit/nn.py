...
class Model:
    ...
    Model subclasses are :ref:`PyTrees <pytrees>`, which means that various
    ...
    """
    def __call__(self, arg: CoopVec[Any], /) -> CoopVec[Any]:
        """
    ...
    def alloc(self, dtype: Type[drjit.ArrayBase], size: int = -1) -> Model:
        """
...
class Linear(Model):
    ...
    The method :py:func:`Model.alloc` initializes the underlying coefficient
    ...
    def _alloc(self, dtype: Type[drjit.ArrayBase], size : int = -1, /) -> Tuple[Model, int]:
        ...
        if in_features == -1 or out_features == -1:
            raise RuntimeError("The network contains layers with an unspecified "
                               "size. You must specify the input size to drjit.nn.Model.alloc().")
