...
class Generator:
    ...
    def __repr__(self) -> str:
        ...
        return f'{type(self).__name__}[seed={seed}, counter={counter}]'

    def __init__(self, seed: ArrayOrInt) -> None:
        """
        Initialize the generator with the given seed.

        Different seeds produce statistically independent sequences of random
        variates.

        Args:
            seed: Seed value (Python int or Dr.Jit UInt64 array).
        """
        self._seed = seed
        self._counter = 0
    ...
    def normal(self, dtype: typing.Type[ArrayT], shape: Shape) -> ArrayT:
        """
...
class Philox4x32Generator(Generator):
    ...
    def __init__(self, seed: ArrayOrInt):
        super().__init__(seed)
    ...
    def _sample(self, dtype: typing.Type[ArrayT], shape: Shape, fn_name: str) -> ArrayT:
        ...
        if is_jit:
            ...
            if not dr.flag(dr.JitFlag.SymbolicScope):
    ...
    def random(self, dtype: typing.Type[ArrayT], shape: Shape) -> ArrayT:
        ...
        if tp == dr.VarType.Float16:
            ...
            raise Exception('Unsupported "dtype": must be a Dr.Jit float16/float32/float64 array.')
