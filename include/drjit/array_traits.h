...
namespace detail {
    ...
    template <typename T> using is_amd_det             = std::enable_if_t<T::IsDrJit && T::Derived::IsAMD>;
...
template <typename T>
constexpr bool is_amd_v = is_detected_v<detail::is_amd_det, T>;
template <typename T> using enable_if_amd_array_t = enable_if_t<is_amd_v<T>>;
...
namespace detail {
    ...
    template <typename T> struct backend<T, enable_if_amd_array_t<T>> {
        static constexpr JitBackend value = JitBackend::AMD;
    };
}
