...
struct Complex : StaticArrayImpl<Value_, 2, false, Complex<Value_>> {
    ...
    ENOKI_INLINE static Complex full_(const Scalar &value, size_t size) {
        return Array<Value_, 2>::full_(value, size);
    }
};

template <typename T> ENOKI_INLINE T real(const Complex<T> &z) { return z.x(); }
template <typename T> ENOKI_INLINE T imag(const Complex<T> &z) { return z.y(); }
...
ENOKI_NOINLINE Stream &operator<<(Stream &os, const Complex<T> &z) {
