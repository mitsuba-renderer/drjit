/*
    enoki/tensor.h -- Tensorial wrapper of a dynamic 1D array
...
ek_vector<size_t> tensor_broadcast(const char *op, T0 &t0, T1 &t1) {
    size_t t0d = t0.ndim(), t1d = t1.ndim(),
           ndim = enoki::max(t0d, t1d);

    if ((t0d != ndim && t0d != 0) || (t1d != ndim && t1d != 0))
        enoki_raise("enoki::Tensor::%s(): incompatible tensor dimensions "
...
ek_vector<size_t> tensor_broadcast(const char *op, T0 &t0, T1 &t1, T2 &t2) {
    size_t t0d = t0.ndim(), t1d = t1.ndim(), t2d = t2.ndim();
    size_t ndim = enoki::max(enoki::max(t0d, t1d), t2d);
    ...
    ek_vector<size_t> shape(ndim, 0);
    for (size_t i = 0; i < ndim; ++i)
        shape[i] = enoki::max(enoki::max(t0d > 0 ? t0.shape(i) : 0,
                                         t1d > 0 ? t1.shape(i) : 0),
                                         t2d > 0 ? t2.shape(i) : 0);
...
struct Tensor
    ...
    ENOKI_ARRAY_IMPORT(Tensor, Base)
    ...
    Tensor(const Array &data) : m_array(data) {
        ...
        if (size != 0 && size != 1)
            enoki_raise("Tensor(): initialization with a non-trivial array "
    ...
    size_t shape(size_t i) const {
        if (i >= m_shape.size())
            enoki_raise("Tensor::shape(%zu): out of bounds!", i);
        return m_shape[i];
    }
