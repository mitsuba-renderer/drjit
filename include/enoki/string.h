/*
    enoki/string.h -- Simple and efficient buffer class for constructing
...
struct StringBuffer {
    ...
    StringBuffer &put(char c) {
        if (ENOKI_UNLIKELY(m_cur + 1 >= m_end))
            expand();
    ...
    StringBuffer &fmt(const char *format, ...) {
        do {
            ...
            if (ENOKI_LIKELY(written + 1 < size)) {
                m_cur += written;
                return *this;
            }
...
private:
    ...
    ENOKI_NOINLINE void expand(size_t minval = 2) {
