...
struct Quaternion : StaticArrayImpl<Value_, 4, false, Quaternion<Value_>> {
    ...
    ENOKI_ARRAY_DEFAULTS(Quaternion)
    ...
    ENOKI_INLINE Quaternion(Value_ &&vi, Value_ &&vj, Value_ &&vk, Value_ &&vr)
...
ENOKI_NOINLINE Stream &operator<<(Stream &os, const Quaternion<T> &q) {
    if constexpr (is_array_v<T>) {
        os << "[";
