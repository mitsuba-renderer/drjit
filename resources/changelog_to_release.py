#!/usr/bin/env python3
"""Turn a section of docs/changelog.rst into GitHub release notes.
...
Markdown, which is what the GitHub release page expects::
    ...
    python3 resources/changelog_to_release.py 1.5.0
...
RTD = "https://drjit.readthedocs.io/en"

# Section heading, e.g. "DrJit 1.5.0 (August 7, 2026)" over a row of dashes.
HEADING_RE = re.compile(r"^DrJit (\d+\.\d+\.\d+) \(([^)]*)\)\s*$")

# Sphinx label definition, e.g. ".. _cache_config:".
LABEL_RE = re.compile(r"^\.\. _([\w.-]+):\s*$", re.MULTILINE)

# Python domain role with an optional explicit target:
#   :py:func:`dr.median() <median>`   or   :py:attr:`ReduceOp.Min`
...
# Cross-reference to a labelled section, e.g. :ref:`cooperative vectors <coop_vec>`.
...
def extract(text: str, version: str) -> tuple[str, str]:
    """Return the (body, date) of the changelog entry for ``version``."""
    ...
    for i, line in enumerate(lines):
        ...
        if start is None and m.group(1) == version:
            start, date = i + 2, m.group(2)  # skip heading + underline
        elif start is not None:
            return "\n".join(lines[start:i]).strip("\n"), date

    if start is None:
        raise SystemExit(f"error: no changelog entry for version {version}")

    return "\n".join(lines[start:]).strip("\n"), date


def label_map() -> dict[str, str]:
    """Map each Sphinx label to the document that defines it."""
    labels: dict[str, str] = {}
    for path in sorted(DOCS.glob("*.rst")):
        for m in LABEL_RE.finditer(path.read_text()):
            labels.setdefault(m.group(1), path.stem)
    return labels
...
def convert(body: str, version: str, labels: dict[str, str]) -> str:
    ...
    def py_role(m: re.Match) -> str:
        ...
        return f"[`{text}`]({base}/reference.html#{target})"
    ...
    def ref_role(m: re.Match) -> str:
        ...
        if label is None:
            return text
        doc = labels.get(label)
        if doc is None:
            return text
        anchor = label.replace("_", "-")
        return f"[{text}]({base}/{doc}.html#{anchor})"
...
def unwrap(body: str) -> str:
    ...
    the changelog would otherwise survive into the published notes.
    """
    out: list[str] = []
...
def main() -> int:
    ...
    parser.add_argument("version", help="release to convert, e.g. 1.5.0")
    ...
    cmd = ["gh", "release", "create", tag, "--verify-tag",
           "--title", f"Release ({tag})", "--notes-file", "-"]
    if args.draft:
        cmd.append("--draft")
